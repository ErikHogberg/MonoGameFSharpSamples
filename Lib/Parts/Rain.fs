...
type Expiry = 
    ...
    new (timeRemaining: float32) = {
        TimeRemaining = timeRemaining
    }


type Raindrop (velocity: Vector2, size: float32, startY: float32) as x =
...
type RainExpirySystem () =
    ...
    override this.Process(gameTime: GameTime, enityId: int) = 
        ...
        expiry.TimeRemaining <- expiry.TimeRemaining - gameTime.GetElapsedSeconds()
        if expiry.TimeRemaining <= 0f then
            this.DestroyEntity(enityId)
        ()

type RainfallSystem(boundaries: Rectangle, minRate: float, maxRate: float, rateIsPerWidth: bool) as x =
    ...
    member this.CreateRaindrop(position: Vector2, velocity: Vector2, size:float32) =
        let mutable entity = this.CreateEntity()
    ...
    override this.Update(gameTime: GameTime) = 
        ...
        for entityId in this.ActiveEntities do
            ...
            raindrop.Velocity <- new Vector2(0f,30f) * dt
            transform.Position <- transform.Position + (raindrop.Velocity + new Vector2(WindStrength, 0f)) * dt

            let dropHitBox = Box.Contains(transform.Position.ToPoint())
            ...
            if ((transform.Position.Y >= float32 boundaries.Bottom || dropHitBox) && (not hasExpiry)) then
                for i in 0..3 do 
                    let velocity = new Vector2(random.NextSingle(-100f, 100f), -raindrop.Velocity.Y * random.NextSingle(0.1f, 0.2f));
                    // var id = CreateRaindrop(transform.Position.SetY(splashSpawnY), velocity, (i + 1) * 0.5f);
                    let id = this.CreateRaindrop(transform.Position.SetY(transform.Position.Y - 1f), velocity, (float32 i + 1f) * 0.5f);
        ...
        if spawnDelay <= 0f then
            for q in 0..50 do
                let position = new Vector2(
                    ...
                    float32 boundaries.Top + random.NextSingle(-240f, -480f)
                    )
                let id = this.CreateRaindrop(position, Vector2.Zero ,random.NextSingle(2f,4f))
                ()
